...
library(C50)
data() #iris is the default data set in R
data("iris") # use iris
View(iris)
...
irisc5.0_train <- C5.0(iris_train[,-5],iris_train$Species) # [] these are used to delete some rows or columns 
windows()
...
iris_tree <- tree(Species~.,data=iris_train) #tree function
plot(iris_tree) #for plotting 
text(iris_tree,pretty = 0) # for the text to include
...
CrossTable(iris_test$Species,pred_tree$final) #71/75 accuracy from crosstable for true positives and true negatives 
