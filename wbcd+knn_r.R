...
round(prop.table(table(wbcd$diagnosis))*100,1) #to get the proportion of B & M 
...
#test normalization
norm(c(1,2,3,4,5)) #to check the normally functioning or not
norm(c(10,20,30,40,50))
#Apply the normalization function to wbcd dataset
wbcd_n <- as.data.frame(lapply(wbcd[2:31], norm)) #making into data.frame formate
View(wbcd_n)
